...
type FSharpKernel() as this =
    ...
    override this.AddScriptReferences(packageReferences: IReadOnlyList<ResolvedPackageReference>) =
        ...
        for packageReference in packageReferences do
            for assemblyPath in packageReference.AssemblyPaths do
                sb.AppendFormat("#r \"{0}\"" + Environment.NewLine) |> ignore

//        this.SubmitCodeAsync(sprintfn "#r \"%s\"")

//        raise (new NotSupportedException(sb.ToString))
        ()
